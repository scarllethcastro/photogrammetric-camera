...
void main() {
  ...
  if (all(greaterThan(testBorder,vec2(0.))))
  {
    ...
    finalColor = texture2D(textures[0], texCoord.xy);
    finalColor += texture2D(textures[1], texCoord.xy);
    finalColor += texture2D(textures[2], texCoord.xy);
    finalColor *= normalization;
  } else {
